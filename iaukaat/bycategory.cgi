...
if ($login && $catt ne "") {
	print '<html lang="en-US">
		...
		$sth = $dbh->prepare("select amount, account,new_value,date, prev_value,type from transactioninfo where userid='$getuser' and category='$catt' ORDER BY tid ASC");
		$sth->execute();
		my %dataexp;
		my %datainc;
		my %alldata;
		...
		$sth = $dbh->prepare("select tid,date,time, amount, account, prev_value, new_value, type, category, tags,comment from transactioninfo where userid='$getuser' AND category='$catt' ORDER BY tid DESC");
		$sth->execute();
