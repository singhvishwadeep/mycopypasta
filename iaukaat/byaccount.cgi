...
if ($login && $account ne "") {
	print '<html lang="en-US">
		<head>
			...
			my $sth = $dbh->prepare("select new_value,date from transactioninfo where tid IN (select max(tid) from transactioninfo where userid='$getuser' AND account='$account' GROUP by date)");
			$sth->execute();
			my %data;
			my @refarr;
		...
		$sth = $dbh->prepare("select tid,date,time, amount, account, prev_value, new_value, type, category, tags,comment from transactioninfo where userid='$getuser' AND account='$account' ORDER BY tid DESC");
		$sth->execute();
