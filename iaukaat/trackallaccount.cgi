	...
	print '<html lang="en-US">
		...
		<body>
			<table class="box" align="center" width="65%">
				...
				my $query = "select id, myusername, name, loggedin,activeaccount from userdatabase ORDER BY id ASC";
				my $sth = $dbh->prepare($query);
				$sth->execute();
				print "<text class=\"fontdec\"><b>Registered users</b></text><br/><br/>";
				print "<table border=\"1\" width=\"100%\">";
				print "<tr><td>S.No.</td><td>ID</td><td>username</td><td>name</td><td>Account Status</td><td>Online Status</td></tr>";
				my $i = 0;
				my $olu = 0;
				while (my $ref = $sth->fetchrow_hashref()) {
					$i++;
					my $id = $ref->{'id'};
					my $myusername = $ref->{'myusername'};
					my $name = $ref->{'name'};
					my $loggedin = $ref->{'loggedin'};
					my $activeaccount = $ref->{'activeaccount'};
					my $msg = "";
					my $bg = "white";
					if ($loggedin eq "1") {
						$olu++;
					...
					my $msg1 = "";
					my $bg1 = "white";
					if ($activeaccount eq "1") {
						$msg1 = "Active Account";
						$bg1 = "green";
					} else {
						$msg1 = "Deactived Account";
						$bg1 = "red";
					}
					
					print "<tr><td>$i</td><td><a href=\"profile.cgi?id=$id\" target=\"_blank\">$id</a></td><td>$myusername</td><td>$name</td><td><text style=\"background:$bg1\">$msg1</text></td><td><text style=\"background:$bg\">$msg</text></td></tr>";
				}
				$sth->finish();
				$dbh->disconnect();
				print "</table>";
				print "<text class=\"fontdec\"><b>$olu users are online now.</b></text><br/>";
