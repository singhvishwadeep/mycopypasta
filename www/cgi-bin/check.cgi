...
$cookie1 = $query->cookie(-name=>'MY_COOKIE',
...
my $CGISESSID = cookie('MY_COOKIE');

if ($CGISESSID ne "") {
	print $query->h3("Cookie found cookie $cookie('MY_COOKIE') -> $CGISESSID");
} else {
	print $query->h3("Cookie not found cookie $cookie('MY_COOKIE') -> $CGISESSID");
}
