	...
	print '<html lang="en-US">
		...
		<body>
			<table class="box" align="center" width="65%">
				...
				my $query = "select ip, date, http_agent, count from index_ip_track ORDER BY date DESC";
				my $sth = $dbh->prepare($query);
				$sth->execute();
				
				print "<table border=\"1\">";
				print "<tr><td>S.No.</td><td>IP</td><td>Date</td><td>http agent</td><td>count</td></tr>";
				my $i = 0;
				while (my $ref = $sth->fetchrow_hashref()) {
					$i++;
					my $ip = $ref->{'ip'};
					my $date = $ref->{'date'};
					my $http_agent = $ref->{'http_agent'};
					my $count = $ref->{'count'};
					print "<tr><td>$i</td><td><a href=\"http://www.ip-tracker.org/locator/ip-lookup.php?ip=$ip\" target=\"_blank\"> $ip </a></td><td>$date</td><td>$http_agent</td><td>$count</td></tr>";
				}
				$sth->finish();
				$dbh->disconnect();
				print "</table>";
