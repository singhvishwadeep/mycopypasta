	...
	print '<html lang="en-US">
		...
		<body>
			<table class="box" align="center" width="65%">
				...
				print '<tr><td>';
				my @accounts = param('list');
				foreach my $acc (@accounts) {
				    my $query = "update userdatabase set activeaccount=1 where id='$acc'";
					my $sth = $dbh->prepare($query);
					$sth->execute();
					print "<a href=\"profile.cgi?id=$acc\" target=\"_blank\">$acc</a> Account got activated<br/>";
				}
