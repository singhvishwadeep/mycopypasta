		...
		<table class="box" align="center" width="65%">
			...
			if ($showmycategory ne "") {
				if ($login == 1) {
						...
						$query = "SELECT id,username,category,topic,discussion,source,tags,date,public FROM datasubmission where (user='$getuser' OR public=1) AND showme=1 AND category='$showmycategory' ORDER BY id DESC";
						$sth = $dbh->prepare($query);
						$sth->execute();
						my $total_results = $sth->rows;
						
						$sth = $dbh->prepare("SELECT id,username,category,topic,discussion,source,tags,date,public FROM datasubmission where (user='$getuser' OR public=1) AND showme=1 AND category='$showmycategory' ORDER BY id DESC  LIMIT $limit OFFSET $offset");
						$sth->execute();
						...
						if ($offset == 0) {
							...
							$prevlink = "categoryview.cgi?limit=$limit&offset=$temp";
						}
						...
						$query = "SELECT id,username,category,topic,discussion,source,tags,date,public,user FROM datasubmission where public=1 AND showme=1 AND category='$showmycategory' ORDER BY id DESC";
						$sth = $dbh->prepare($query);
						$sth->execute();
						my $total_results = $sth->rows;
						
						$sth = $dbh->prepare("SELECT id,username,category,topic,discussion,source,tags,date,public,user FROM datasubmission where public=1 AND showme=1 AND category='$showmycategory' ORDER BY id DESC LIMIT $limit OFFSET $offset");
						$sth->execute();
						my $current_results = $sth->rows;
						my $start = $offset + 1;
						my $end = $offset + $current_results;
